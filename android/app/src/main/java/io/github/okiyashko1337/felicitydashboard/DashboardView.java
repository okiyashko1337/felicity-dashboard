...
final class DashboardView extends View {
    ...
    private int hitTarget(float x,float y){float h=64*scale;if(x>=300*scale&&x<=400*scale&&y>=5*scale&&y<=59*scale)return 40;if(page==-1&&y<h&&x<55*scale)return 22;if(y<h&&x>505*scale&&x<645*scale)return 20;if(page!=-1&&y<h&&x>=55*scale&&x<230*scale)return 21;if(page==-3&&y>=100*scale&&y<166*scale)return 30;if(page==-3&&y>=174*scale&&y<240*scale)return 31;if(page==-3&&y>=248*scale&&y<314*scale)return 32;if(page==-3&&y>=330*scale&&x>=28*scale&&x<322*scale)return 33;if(page!=-1||y<h)return -1;float gap=12*scale,pad=20*scale,top=h+12*scale,cw=(getWidth()-pad*2-gap*2)/3f,ch=(getHeight()-top-18*scale-gap)/2f;int col=(int)((x-pad)/(cw+gap)),row=(int)((y-top)/(ch+gap));return col>=0&&col<3&&row>=0&&row<2?row*3+col:-1;}
    private void tap(float x,float y){
        int target=hitTarget(x,y);if(target<0)return;
        sound();
        if(target==40){if(listener!=null)listener.onCameraRequested();return;}
        float h=64*scale;if(page==-1&&y<h&&x<55*scale){page=-3;invalidate();return;}if(y<h&&x>505*scale&&x<645*scale){page=-2;invalidate();return;}if(page!=-1){if(y<h&&x>=55*scale&&x<230*scale){showHome();return;}if(page==-3&&target==30&&listener!=null){listener.onSettingsRequested();return;}if(page==-3&&target==31&&listener!=null){listener.onWeatherSettingsRequested();return;}if(page==-3&&target==32&&listener!=null){listener.onAjaxSettingsRequested();return;}if(page==-3&&target==33){page=-4;invalidate();return;}return;}

...
public final class CameraActivity extends Activity {
    private LibVLC vlc;private MediaPlayer player;private SurfaceView surface;private TextView clock,stats;private PrivacyButton muteButton,micButton;private ImageView ringSnapshot;private Bitmap ringBitmap;private boolean muted=true,listenEnabled,audioReady,micEnabled,ringEvent;private BackchannelSession backchannel;private AjaxAudioSession inboundAudio;private float zoom=1f,panX,panY,smoothedFps=-1f;private int videoWidth=1024,videoHeight=576,lastPictures;private long lastStatsMs;private OverScroller scroller;
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);ringEvent=getIntent().getBooleanExtra("ring",false);boolean redNight=getIntent().getBooleanExtra("red_night",false);int foreground=redNight?Color.rgb(255,72,52):Color.WHITE,accent=redNight?Color.rgb(210,35,25):Color.rgb(89,222,209),panel=redNight?Color.rgb(30,0,0):Color.rgb(7,17,15);getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(redNight){WindowManager.LayoutParams attributes=getWindow().getAttributes();attributes.screenBrightness=.18f;getWindow().setAttributes(attributes);}getWindow().getDecorView().setSystemUiVisibility(5894);FrameLayout root=new FrameLayout(this);root.setBackgroundColor(Color.BLACK);
        ...
        ringSnapshot=new ImageView(this);ringSnapshot.setScaleType(ImageView.ScaleType.CENTER_CROP);ringSnapshot.setVisibility(View.GONE);android.graphics.drawable.GradientDrawable snapshotFrame=new android.graphics.drawable.GradientDrawable();snapshotFrame.setColor(panel);snapshotFrame.setStroke(3,accent);snapshotFrame.setCornerRadius(12);ringSnapshot.setBackground(snapshotFrame);ringSnapshot.setPadding(3,3,3,3);FrameLayout.LayoutParams rp=new FrameLayout.LayoutParams(250,142,Gravity.BOTTOM|Gravity.LEFT);rp.setMargins(18,0,0,18);root.addView(ringSnapshot,rp);
        View header=new HeaderBackground(this,redNight);root.addView(header,new FrameLayout.LayoutParams(-1,64,Gravity.TOP));Button back=button("‹ ENERGY");back.setTextColor(foreground);back.setTextSize(16);back.setBackgroundColor(Color.TRANSPARENT);FrameLayout.LayoutParams bp=new FrameLayout.LayoutParams(155,58,Gravity.TOP|Gravity.LEFT);bp.setMargins(42,3,0,0);root.addView(back,bp);back.setOnClickListener(v->finish());
        stats=new TextView(this);stats.setText("CONNECTING…");stats.setTextColor(foreground);stats.setTextSize(12);stats.setGravity(Gravity.CENTER);stats.setTypeface(android.graphics.Typeface.DEFAULT_BOLD);stats.setLines(2);FrameLayout.LayoutParams sp=new FrameLayout.LayoutParams(245,58,Gravity.TOP);sp.setMargins(190,3,0,0);root.addView(stats,sp);
        muteButton=new PrivacyButton(this,false,redNight);FrameLayout.LayoutParams mup=new FrameLayout.LayoutParams(54,52,Gravity.TOP);mup.setMargins(442,6,0,0);root.addView(muteButton,mup);muteButton.setOnClickListener(v->toggleMute());
        micButton=new PrivacyButton(this,true,redNight);FrameLayout.LayoutParams mip=new FrameLayout.LayoutParams(54,52,Gravity.TOP);mip.setMargins(502,6,0,0);root.addView(micButton,mip);micButton.setOnClickListener(v->toggleMic());
    ...
    private void toggleMic(){micEnabled=!micEnabled;if(micEnabled){muted=false;listenEnabled=true;beginTalk();}else endTalk();setInboundVolume();updateStatus();}
    private void setInboundVolume(){boolean audible=!muted&&listenEnabled&&backchannel==null;if(player!=null)player.setVolume(audible?100:0);if(inboundAudio!=null)inboundAudio.setVolume(audible?100:0);}
    ...
    private void beginTalk(){if(muted||!micEnabled)return;handler.removeCallbacks(finishRunnable);if(checkSelfPermission(android.Manifest.permission.RECORD_AUDIO)!=PackageManager.PERMISSION_GRANTED){micEnabled=false;requestPermissions(new String[]{android.Manifest.permission.RECORD_AUDIO},52);updateStatus();return;}if(backchannel!=null)return;if(player!=null)player.setVolume(0);android.content.SharedPreferences p=getSharedPreferences("felicity",MODE_PRIVATE);backchannel=new BackchannelSession(this,p.getString("ajax_host","192.168.13.209:8080"),p.getString("ajax_user",""),p.getString("ajax_password",""),new BackchannelSession.Listener(){public void onStarted(){}public void onError(String error){runOnUiThread(()->{micEnabled=false;endTalk();});}});backchannel.start();}
    private void endTalk(){if(backchannel!=null){backchannel.stop();backchannel=null;}setInboundVolume();updateStatus();scheduleFinish();}

...
final class OnvifEventClient implements Runnable {
    ...
    private final String host,user,password;private final Listener listener;private volatile boolean stopped;
    ...
    @Override public void run(){while(!stopped){try{String service="http://"+host+"/onvif/event_service";String created=post(service,create(service),CREATE_ACTION);String endpoint=match(created,"<wsa5:Address>([^<]+)</wsa5:Address>");if(endpoint==null)endpoint=match(created,"<wsa:Address>([^<]+)</wsa:Address>");if(endpoint==null)throw new Exception("No PullPoint address");listener.onListening();while(!stopped){String body=post(endpoint,pull(endpoint),PULL_ACTION);Matcher topics=Pattern.compile("<wsnt:Topic[^>]*>([^<]+)</wsnt:Topic>").matcher(body);while(topics.find()){String topic=topics.group(1);if(meaningful(topic,body)){listener.onEvent(topic);break;}}}}catch(Exception e){if(!stopped)listener.onError(e.getMessage());try{Thread.sleep(5000);}catch(InterruptedException ignored){return;}}}}
    private static boolean meaningful(String topic,String body){if(topic.contains("DebugHud"))return false;if(topic.contains("RingDetector"))return Pattern.compile("Value=\"(?:true|1|active)\"",Pattern.CASE_INSENSITIVE).matcher(body).find();return topic.contains("MotionAlarm")||topic.contains("MotionDetector")||topic.contains("ObjectDetection/Object");}

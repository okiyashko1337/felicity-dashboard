...
final class OnvifReplayProxy implements Closeable {
    ...
    private volatile String challenge="",localBase="",playCseq="";private volatile boolean closed;private volatile long firstPacketTimeMs,rangeStartTimeMs;private Socket client,remote;
    ...
    private void relay(Socket from,Socket to,boolean requestDirection){try{InputStream in=from.getInputStream();OutputStream out=to.getOutputStream();while(!closed){int first=in.read();if(first<0)break;if(first=='$'){byte[] prefix=new byte[3];readFully(in,prefix);int length=((prefix[1]&255)<<8)|(prefix[2]&255);byte[] payload=new byte[length];readFully(in,payload);if(!requestDirection)observeReplayTime(payload);synchronized(out){out.write(first);out.write(prefix);out.write(payload);out.flush();}continue;}byte[] header=readHeader(in,first);String text=new String(header,"ISO-8859-1");int length=contentLength(text);byte[] body=new byte[length];readFully(in,body);String bodyText=length==0?"":new String(body,"ISO-8859-1");String rewritten=requestDirection?rewriteRequest(text):rewriteResponse(text);bodyText=requestDirection?toUpstream(bodyText):toLocal(bodyText);rewritten=setContentLength(rewritten,bodyText.getBytes("ISO-8859-1").length);synchronized(out){out.write(rewritten.getBytes("ISO-8859-1"));out.write(bodyText.getBytes("ISO-8859-1"));out.flush();}}}catch(Exception ignored){}finally{close();}}
    ...
    private String rewriteRequest(String header)throws Exception{
        ...
        if("PLAY".equals(method)){playCseq=headerValue(result,"CSeq");String localRange=headerValue(result,"Range");result=removeHeader(result,"Range");result=insertHeader(result,"Range: clock="+clock(startMs)+"-");result=removeHeader(result,"Rate-Control");result=insertHeader(result,"Rate-Control: yes");result=removeHeader(result,"Scale");result=insertHeader(result,"Scale: 1.0");Log.i("FelicityReplay","RTSP PLAY · local="+localRange+" · upstream="+headerValue(result,"Range")+" · require="+headerValue(result,"Require")+" · rate="+headerValue(result,"Rate-Control"));}
    ...
    private String rewriteResponse(String header){String auth=headerValue(header,"WWW-Authenticate");if(auth!=null&&!auth.isEmpty())challenge=auth;String range=headerValue(header,"Range"),rtp=headerValue(header,"RTP-Info"),cseq=headerValue(header,"CSeq");long rangeTime=ReplayClock.fromRange(range);if(rangeTime>0&&!playCseq.isEmpty()&&playCseq.equals(cseq))rangeStartTimeMs=rangeTime;if(range!=null||rtp!=null)Log.i("FelicityReplay","RTSP response · status="+line(header,0)+" · range="+range+" · rtp="+(rtp==null?"—":"present"));return toLocal(header);}
    private void observeReplayTime(byte[] packet){long time=ReplayClock.fromRtp(packet);if(time>0&&firstPacketTimeMs==0){firstPacketTimeMs=time;Log.i("FelicityReplay","First ONVIF frame time · "+time);}}

...
Клиент завершает каждый локальный TCP-запрос подтверждением `.` и закрывает
сессию сразу после последнего пакета. Штатный двухсекундный опрос сохраняется.
Только если модуль перестаёт отвечать, частота повторов автоматически снижается
вплоть до одного запроса в минуту, а после успешного ответа возвращается к двум
секундам.

...
строятся по компактным суточным итогам в SQLite. В ответе `/api/current`
дополнительно доступны пофазные токи, частота, DC-шина, мощность резервного
выхода и полный сырой пакет устройства в поле `raw`.
